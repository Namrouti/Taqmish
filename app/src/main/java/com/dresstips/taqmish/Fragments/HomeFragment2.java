...
public class HomeFragment2 extends Fragment implements ClosetAdapterHomeFragemntIINterface,
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        colorSlider.setOnKeyListener(new View.OnKeyListener() {
            ...
            public boolean onKey(View v, int keyCode, KeyEvent event) {
                if (keyCode == event.KEYCODE_VOLUME_UP || keyCode == event.KEYCODE_VOLUME_DOWN) {
                    DialogFragment dialog = new SeekBarDialog(HomeFragment2.this.getContext(), 255, 0, colorRange, "Increase to get more result", HomeFragment2.this);
                    dialog.show(HomeFragment2.this.getActivity().getSupportFragmentManager(), "");
                    return true;
                }
                return false;
            }
        ...
        outfit = new OutfitClass();
        topimage = view.findViewById(R.id.topItem);
        downimage = view.findViewById(R.id.downItem);
        watch = view.findViewById(R.id.watchItem);
        shoos = view.findViewById(R.id.shoosItem);
        ...
        mDBRef.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                for (DataSnapshot d : snapshot.getChildren()) {
                    ...
                    HomeFragment2.this.data.add(sc);
                    HomeFragment2.this.mAdapter.notifyItemChanged(HomeFragment2.this.data.size() - 1);
                    mAdapter.dataCanged();
                }
...
//the item of closets recycle
    ...
    public void itemClicked(SiteClosets sc, ImageView iv) {
        ...
        if (sc.getBodyPart().equals("الجزء العلوي")) {
            ...
            Picasso.with(HomeFragment2.this.getContext()).load(sc.getFilePath()).fit().into(topimage);
            outfit.setTop(sc);
            ...
            Picasso.with(HomeFragment2.this.getContext()).load(sc.getFilePath()).fit().into(downimage);

        } else if (sc.getBodyPart().equals("اكسسوارات")) {
            if (sc.getSubParts().equals("ساعه")) {
                Picasso.with(HomeFragment2.this.getContext()).load(sc.getFilePath()).fit().into(watch);
                outfit.setAccessories(sc);

            } else if (sc.getSubParts().equals("حذاء")) {
                Picasso.with(HomeFragment2.this.getContext()).load(sc.getFilePath()).fit().into(shoos);
                outfit.setShoes(sc);
            }
    ...
    private void showSaveToCalenderDialog() {
        DialogFragment df = new saveOutfitDialog(outfit);
        df.show(this.getActivity().getSupportFragmentManager(), "Save to Calendar items");
    }
